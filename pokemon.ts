...
data.forEach((a, index) => { // data 배열 안에 있는 각각의 객체 요소들을 pokemon 파라미터로 가져오기
  ...
  let inform:any = document.createElement('div'); // information 넣을 inform div 태그 선언
  let h2Text:any = document.createElement('h2'); // 1. h2 태그 선언
  let pHeight:any = document.createElement('p'); // 2. Height 넣는 p 태그 선언
  let pWeight:any = document.createElement('p'); // 3. Weight 넣는 p 태그 선언
  let pTypes:any = document.createElement('p'); // 4. type 넣는 p 태그 선언
